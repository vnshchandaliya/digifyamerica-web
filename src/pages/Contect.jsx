  ...
  const handleSubmit = async (e) => {
    ...
    try {
     const response = await fetch("http://localhost:5000/send-email", {
      ...
      if (response.ok) {
        alert("Test successful! Data received on backend.");
      } else {
        alert("Test failed! Something is wrong with the connection.");
      }
    } catch (error) {
              ...
              <div>
                ...
                <div className="mt-1">
                  <input
                    type="text"
                    id="firstName"
                    name="name" // Changed name to 'name' for simplicity in backend
